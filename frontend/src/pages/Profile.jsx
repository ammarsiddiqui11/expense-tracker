...
export default function Profile() {
  ...
  const changePasswordHandler = async () => {
    ...
    try {
      // Call STEP 1 Endpoint
      const res = await axios.put("/auth/change-password/request-otp", {
        oldPassword,
        newPassword,
      });

      // On Success: Navigate to Verify OTP Page
      // We pass 'userId' and 'flow' via router state
      navigate(`/verify-otp?userId=${res.data.userId}&flow=change`);
      
      // Close modal (optional since we are navigating away)
